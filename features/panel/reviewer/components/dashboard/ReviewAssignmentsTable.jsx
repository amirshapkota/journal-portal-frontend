  ...
  const columns = [
    {
      ...
      header: 'Submission #',
      render: (row) => row.submission_details?.submission_number || row.submission_number || '-',
    },
    {
      key: 'submission_title',
      header: 'Submission Title',
      render: (row) => (
        <EllipsisTooltip text={row.submission_title || row.submission_details?.title || 'N/A'} />
      ),
    },
    {
      key: 'journal_name',
      ...
      render: (row) => <EllipsisTooltip text={row.submission_details?.journal?.title || 'N/A'} />,
    },

    {
      ...
      header: 'Status',
      render: (row) => (
        <StatusBadge status={row.status} statusConfig={reviewAssignmentStatusConfig} />
      ),
    },
    {
      key: 'due_date',
      header: 'Due Date',
      render: (row) => (
        <span className="text-sm">{format(new Date(row.due_date), 'MMM dd, yyyy')}</span>
      ),
    },
    {
      key: 'days_remaining',
      ...
      render: (row) => (
        <div
          className={`text-sm font-semibold ${
            row.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'
          }`}
        >
          {row.is_overdue
