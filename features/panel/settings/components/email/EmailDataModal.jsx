...
export default function EmailDetailModal({ email, open, onOpenChange }) {
  ...
  const displayDate = email.sent_at ? new Date(email.sent_at) : new Date(email.created_at);
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-foreground">{email.template_type.replace(/_/g, ' ')}</p>
            </div>
            <div>
            ...
            <div className="border-l-4 border-red-500 bg-red-500/10 p-4 rounded">
              <div className="flex gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-700 dark:text-red-400">{email.error_message}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="border-t border-border pt-4">
            <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">Email ID</p>
            <p className="font-mono text-xs text-muted-foreground break-all">{email.id}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
