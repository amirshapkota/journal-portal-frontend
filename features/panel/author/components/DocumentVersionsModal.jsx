...
export default function DocumentVersionsModal({ open, onOpenChange, documentId }) {
  const {
    data: versions,
    isPending: loading,
    error,
      ...
      <DialogContent className="md:max-w-[85%] lg:max-w-[55%] max-h-[90vh] overflow-y-auto p-5 sm:p-6">
        ...
        {error && (
          <div className="text-center py-8">
            <p className="text-destructive">{error.message || 'Failed to load versions'}</p>
          ...
          <div className="space-y-4">
            {versions.versions.map((version, index) => (
              <div key={version.id}>
                <div className="flex items-start justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                  <div className="flex-1 space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      {version.immutable_flag && <Badge variant="secondary">Locked</Badge>}
                    </div>
                ...
                {index < versions.versions.length - 1 && <Separator className="my-4" />}
              </div>
            ))}
