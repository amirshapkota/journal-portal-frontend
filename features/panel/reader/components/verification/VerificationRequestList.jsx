    ...
    <Card key={request.id} className="gap-3">
      <CardHeader>
        <div className="flex items-start justify-between">
          ...
          {getStatusBadge(
            request.status,
            request.status === 'INFO_REQUESTED' ? () => setRespondRequest(request) : undefined
          )}
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            <p className="text-sm font-medium text-muted-foreground">Institutional Email</p>
            <p className="text-sm">{request.affiliation_email}</p>
          </div>
          <div className="space-y-1">
            <p className="text-sm font-medium text-muted-foreground">Academic Position</p>
            <p className="text-sm capitalize">{request.academic_position}</p>
          </div>
          <div className="space-y-1">
            <p className="text-sm font-medium text-muted-foreground">ORCID Verified</p>
            <p className="text-sm">
        ...
        {request.rejection_reason && (
          <div className="mt-2 p-2 rounded-lg bg-destructive/10">
            <p className="text-xs font-medium capitalize text-destructive mb-1">Rejection Reason</p>
            <p className="text-sm capitalize text-muted-foreground">{request.rejection_reason}</p>
          </div>
        )}
        ...
        {request.status === 'INFO_REQUESTED' && (
          <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/30 border border-blue-300 dark:border-blue-800">
            <p className="text-sm text-blue-600 dark:text-blue-100">
              <span className="font-semibold">Note:</span> If the additional requested info is not
              part of the form (eg. connect ORCID), complete it first, then click ‘Respond’ and
              submit the form without any changes to notify the admin.
            </p>
          </div>
        )}
        {(request.status === 'PENDING' || request.status === 'INFO_REQUESTED') && (
          <Button
            ...
            disabled={withdrawMutation.isPending && withdrawMutation.variables === request.id}
          >
            {withdrawMutation.isPending && withdrawMutation.variables === request.id
              ? 'Withdrawing...'
              : 'Withdraw Request'}
          </Button>
        )}
      ...
      {rest.length > 0 && (
        <Accordion type="single" collapsible className="mt-2 ">
          <AccordionItem value="older-requests">
            ...
            <AccordionContent className="space-y-4">{rest.map(renderRequestCard)}</AccordionContent>
          </AccordionItem>
        </Accordion>
      )}
      {respondRequest && (
