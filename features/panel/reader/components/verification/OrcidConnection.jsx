...
const OrcidConnection = () => {
  ...
  const { mutate: disconnectOrcidMutation, isPending: isDisconnecting } = useDisconnectOrcid();
  ...
  const handleDisconnectOrcid = () => {
    disconnectOrcidMutation(undefined, {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries(['orcid-status', 'verification-requests']);
      },
      onError: (error) => {
  ...
  return (
    <Card className="border-border dark:border-slate-700">
      <CardHeader>
        ...
        <CardDescription>
          Link your ORCID profile for verified identity and automatic information retrieval
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoadingStatus ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-muted-foreground">Loading ORCID status...</span>
          </div>
        ) : isErrorStatus ? (
          <ErrorCard
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
              <div className="space-y-2">
                ...
                <p className="text-md  text-foreground">{orcidStatus.orcid_id}</p>
              </div>
              <div className="space-y-2">
                <p className="text-sm font-medium text-muted-foreground">Status</p>
                <Badge className="bg-emerald-600 dark:bg-emerald-700">{orcidStatus.status}</Badge>
              </div>
              {orcidStatus.token_scope && (
                <div className="space-y-2">
                  <p className="text-sm font-medium text-muted-foreground">Token Scope</p>
