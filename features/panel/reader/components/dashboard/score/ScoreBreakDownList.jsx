...
export function ScoreBreakdownList({ scoreBreakdown }) {
  if (!scoreBreakdown || scoreBreakdown.length === 0) {
    return (
      <div className="text-center py-4 text-muted-foreground">No score breakdown available</div>
    );
  }
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'completed':
        return <CheckCircle2 className="h-5 w-5 text-green-600 dark:text-green-500 shrink-0" />;
      case 'missing':
        return <XCircle className="h-5 w-5 text-muted-foreground/50 shrink-0" />;
      case 'partial':
        return <AlertCircle className="h-5 w-5 text-yellow-600 dark:text-yellow-500 shrink-0" />;
      default:
        return <XCircle className="h-5 w-5 text-muted-foreground/50 shrink-0" />;
    }
  };
