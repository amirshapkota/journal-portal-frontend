...
const journalSchema = z.object({
  ...
  editor_in_chief_email: z.string().email('Must be a valid email').optional().or(z.literal('')),
...
const defaultValues = {
  ...
  website_url: '',
  contact_email: '',
  editor_in_chief_email: '',
  is_active: false,
  is_accepting_submissions: true,
};

export function AdminJournalFormModal({ isOpen, onClose, onSave, isLoading = false }) {
  const form = useForm({
    resolver: zodResolver(journalSchema),
    defaultValues,
  });

  // Fetch editor users for editor-in-chief selection
  const {
    data: usersData,
  ...
  const userOptions = profiles.map((profile) => ({
    ...
    label: `${profile.profile.display_name || profile.profile.user_name || 'No name'} (${profile.profile.user_email})`,
  }));
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <FormField
                  ...
                  name="editor_in_chief_email"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Editor-in-Chief Email</FormLabel>
                      <FormControl>
                        <SearchableSelect
                          options={userOptions}
                          value={field.value}
                          onChange={field.onChange}
                          placeholder={loadingUsers ? 'Loading users...' : 'Select Editor-in-Chief'}
                          emptyText={
                            usersError
                              ? 'Error loading users'
                              : userOptions.length === 0
                                ? 'No users found with EDITOR role'
                                : 'No user found.'
                          }
                      ...
                      <FormItem className="flex items-center justify-between space-y-0">
                        <div className="space-y-0.5">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Make journal visible and active
                          </p>
                        </div>
                        <FormControl>
