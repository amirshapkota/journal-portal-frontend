    ...
    },
    {
      ...
      render: (row) => (
        <Button
          ...
          size="sm"
          onClick={() =>
            router.push(`/editor/assigned-journals/${journalId}/submissions/${row.id}`)
          }
        >
  ...
  if (journalError) {
    return (
      <ErrorCard
        ...
        description={journalError?.message || 'Unable to load journal details. Please try again.'}
      ...
      <div className="flex items-center justify-between">
        <div className="space-y-1">
          ...
          <div className="flex items-center gap-3">
            ...
            {myRole && (
              <Badge variant="outline" className={getRoleBadgeColor(myRole.role)}>
                {myRole.role_display}
              </Badge>
            )}
          </div>
          <p className="text-muted-foreground">View and manage submissions for this journal</p>
        </div>
      </div>
