...
import { ArrowLeft, FileText, Calendar, User, Loader2, Eye } from 'lucide-react';
import {
  DataTable,
  ErrorCard,
  LoadingScreen,
...
import { useGetJournalById, useGetJournalSubmissions } from '@/features/panel/editor/journal';
...
export default function JournalSubmissionsPage() {
  ...
  const { progressData, isPolling, startPolling, isWaitingForStart, stoppedByError, error } =
  ...
  if (journalError) {
    return (
      <ErrorCard
        ...
        description={
          journalError?.message || journalError?.toString() || 'Error loading journal details.'
        }
  ...
  if (submissionsError) {
    return (
      <ErrorCard
        ...
        description={
          submissionsError?.message || submissionsError?.toString() || 'Error loading submissions.'
        }
        onRetry={refetch()}
      />
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-0 sm:items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-muted-foreground">View and manage all submissions for this journal</p>
        </div>
            ...
            {!isImportActive ? (
              <>
                ...
                <ConfirmationPopup
                  ...
                  loadingText={
                    isWaitingForStart ? 'Waiting for import to start...' : 'Starting import...'
                  }
                  autoClose={false}
                />
              </>
            ) : (
              <Button onClick={handleViewProgress} variant="secondary" size="sm" className="w-fit">
                <Loader2 className="h-4 w-4 mr-2 animate-spin" />
